...
public interface GenericCardGameModel<K> {
   ...
   * are 3 players and 52 cards in the deck, player 0 will get card nos 0,3,6,9,..
   * and player 1 will
   ...
   *
   * This string should be in the following format ({S} is space, {NL} is a newline,
   * text in italics
   * are comments and do not appear in the string). Replace placeholders with actual data as
   * applicable:
   * <pre>
   * Number of players:{S}X{NL}
   * cs3500.hw02.Player{S}1:{S}P11,{S}P12,{S}P13, ...{NL} <i>(cs3500.hw02.Player 1 cards in sorted
   * order)</i>
   * cs3500.hw02.Player{S}2:{S}P21,{S}P22,{S}P23, ...{NL} <i>(cs3500.hw02.Player 2 cards in sorted
   * order)</i>
   * ...
   * cs3500.hw02.Player{S}X:{S}PX1,{S}PX2,{S}PX3, ...{NL} <i>(cs3500.hw02.Player X cards in sorted
   * order)</i>
   * </pre>
   *
